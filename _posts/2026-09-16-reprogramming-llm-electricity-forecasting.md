...
The comparison is in. Chronos here is `chronos-bolt-small`, 48M parameters, T5-based, zero-shot. A from-scratch Transformer, patch-based like LoadCast but with no pretrained backbone, no reprogramming, and no covariates, matches LoadCast's own trainable-parameter count (19.3M against 18.0M) to test whether the pretrained weights are earning their keep at all. Auto-ARIMA (via [statsforecast](https://github.com/Nixtla/statsforecast)) fits a seasonal order per household, then refits it for every evaluation window. LightGBM is a single gradient-boosted-trees model, trained once across all training households on lagged and calendar features. Seasonal-naive just repeats the value from exactly one day earlier.

| Method | MASE | MAE (kWh) |
|---|---|---|
| Chronos (zero-shot) | 0.755 | 0.092 |
| Scratch Transformer | 0.814 | 0.100 |
...
Chronos wins. LoadCast hasn't closed the gap, by about 0.014 kWh per half-hour reading on average across the holdout households. Small per household. Whether it matters at grid scale, aggregated across thousands of day-ahead schedules, isn't something this project measures yet. More surprising: the scratch Transformer beats LoadCast too, by a real margin, with no pretraining, no covariates, and in about a tenth of the training time, 12 minutes against LoadCast's 100. LoadCast does still beat the classical toolkit: Auto-ARIMA, LightGBM, and seasonal-naive all cluster within 0.03 MASE of each other, essentially tied, and LoadCast holds a real margin over all three. But it's no longer the strongest thing that's actually learned from this data. That title currently belongs to a model with no pretrained knowledge at all. The one thing that's helped LoadCast specifically is training on more households. 150, then 450, then 900. Each step improved holdout MASE a bit further. Whether more data alone would ever close the gap to the scratch Transformer, let alone Chronos, isn't tested here.
...
So the general-purpose LLM doesn't win yet, and the accuracy case for it just got harder to make. A from-scratch model with no pretrained knowledge, the same parameter budget, a fraction of the training time, does better. Whatever EuroLLM's pretrained weights contribute here, it isn't beating a model built to do nothing but this one task. There are two open questions now, not one: whether reprogramming ever earns back the accuracy a simpler model gets for free, and whether the language capability, tested above and still rough, is real or just a promise the architecture makes but hasn't kept.
